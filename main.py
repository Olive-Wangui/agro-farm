...
def edit(rid):
    ...
    if request.method=="POST":
        ...
        # query=db.engine.execute(f"UPDATE `register` SET `farmername`='{farmername}',`email`='{email}',`age`='{age}',`gender`='{gender}',`phonenumber`='{phonenumber}',`farming`='{farmingtype}'")
        post=Register.query.filter_by(rid=rid).first()
...
def register():
    ...
    if request.method=="POST":
        ...
        phonenumber=request.form.get('phonenumber')
        farmingtype=request.form.get('farmingtype')     
        query=Register(farmername=farmername, email=email,age=age,gender=gender,phonenumber=phonenumber,farming=farmingtype)
        db.session.add(query)
        db.session.commit()
        # query=db.engine.execute(f"INSERT INTO `register` (`farmername`,`email`,`age`,`gender`,`phonenumber`,`address`,`farming`) VALUES ('{farmername}','{age}','{gender}','{phonenumber}','{farmingtype}')")
